...
$this->title = "View - {$model->cam_title}";
    ...
    <div class="container">
        <div class="row">
            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
                <?php $this->renderPartial('_view_image', compact('model', 'themeUrl')); ?>
                <div class="row">
                    <?php
            ...
            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 course-details">
                <h2> <?php echo $model->cam_title; ?></h2>
                <?php if ($is_tutor) { ?>
                    <button class="btn btn-default edit-btn" data-toggle="modal" data-target=".bs-example-modal-sm2" data-dismiss=".bs-example-modal-sm2" onclick="location.href = '<?php echo Yii::app()->createAbsoluteUrl('/site/cam/update', array('id' => $model->cam_id)) ?>'"> <i class="fa fa-pencil"></i> </button>
                <?php } ?>
                <p class="date"> Created Date : <?php echo date(PHP_SHORT_DATE_FORMAT, strtotime($model->created_at)); ?></p>
                <p><?php echo $model->cam_description; ?></p>
                <h4 class="importants-heading"> Importants </h4>
                <p> <?php echo $model->cam_important; ?> </p>
                <h4> Country </h4>
                <p> <?php echo $tutor->userCountry->country_name; ?></p>
                <h4> Languages </h4>
...
if ($logged_user) {
    ...
    $this->renderPartial('_comments_form', compact('model', 'cam_comments'));
...
$js = <<< EOD
    jQuery(document).ready(function ($) {
        ...
        $('#book_extra_main').on('ifChecked', function(event){
            var newPrice = parseFloat(extra_div.data('cam_price')) + parseFloat(extra_div.data('extra_price'));
            $('.cam_price_txt').html('$ '+newPrice);
            $('#temp_book_extra_inner').iCheck('check'); 
            $('#book_extra_inner').iCheck('check'); 
        });

        $('#book_extra_main').on('ifUnchecked', function(event){
            var newPrice = parseFloat(extra_div.data('cam_price'));
            $('.cam_price_txt').html('$ '+newPrice);
